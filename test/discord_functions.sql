...
BEGIN
	...
	RETURN NEXT is(SELECT Server_Id FROM discord.Servers WHERE Server_Id = 0, 0, 'discord.AddBot must have added a Servers record.');
	...
	RETURN NEXT is(SELECT Server_Id FROM discord.Servers WHERE Server_Id = 0, 0, 'discord.AddBot must keep existing Servers records.');
END;
$$ LANGUAGE plpgsql;
