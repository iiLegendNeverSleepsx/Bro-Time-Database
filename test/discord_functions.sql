...
BEGIN
	...
	RETURN NEXT results_eq($sql$SELECT Namespace, Value, User_Id FROM discord.User_Settings WHERE Namespace = 'testD';$sql$, $sql$SELECT 'testD' AS Namespace, '{}'::jsonb AS Value, 0::bigint AS User_Id;$sql$,  'discord.SetSettings must have added a User_Settings record.');
	RETURN NEXT results_eq($sql$SELECT Namespace, Value, User_Id FROM discord.User_Settings WHERE Namespace = 'testE';$sql$, $sql$SELECT 'testE' AS Namespace, '{}'::jsonb AS Value, 0::bigint AS User_Id;$sql$,  'discord.SetSettings must have added a User_Settings record.');
