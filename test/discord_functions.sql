...
BEGIN
	...
	RETURN NEXT lives_ok('SELECT discord.AddBot(null) FOR UPDATE;', 'discord.AddBot should not accept null arguments.');
	-- Creates a Servers record.
	RETURN NEXT throws_ok('SELECT discord.AddBot(0) FOR UPDATE;', 'discord.AddBot must not throw if called correctly.');
	...
	RETURN NEXT throws_ok('SELECT discord.AddBot(0) FOR UPDATE;', 'discord.AddBot must not throw if called again.');
